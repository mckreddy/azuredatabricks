...
articlesByMichaelDF = databricksBlogDF.select("title").filter(array_contains("authors", "Michael Armbrust"))
...
# ANSWER
uniqueCategoriesDF = databricksBlogDF.select(explode(col("categories")).alias("category")).orderBy("category").distinct()
...
display(uniqueCategoriesDF.orderBy("category"))
